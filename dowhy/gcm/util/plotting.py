...
def plot(causal_graph: nx.Graph,
    ...
    the Python graphviz package (``pip install graphviz``) and the shared system library (e.g. ``brew install
    ...
    **Example usage**::
    ...
    """
    try:
        from dowhy.gcm.util.graphviz import _plot_causal_graph_graphviz
        ...
        try:
            ...
            _logger.info("Graphviz installation not found, falling back to networkx plotting. "
                         "For better looking plots, consider installing graphviz.")
            _plot_causal_graph_networkx(causal_graph,
                                        causal_strengths=causal_strengths,
                                        filename=filename,
                                        display_plot=display_plot,
                                        **kwargs)
        except TypeError as error:  # TODO: Fix graphviz dependency
            if 'render() got an unexpected keyword argument \'format\'' in str(error):
                raise TypeError(
                    str(error) + "\nIt seems an older graphviz version is installed. The plot function using "
                                 "graphviz is currently not supported for graphviz version older than 0.10. "
                                 "Consider updating it to a newer version. Note: Newer autogluon versions "
                                 "do not depend on an old graphviz version anymore.\n"
                                 "If an update to a newer graphviz version is not possible or feasible,"
                                 " consider using the internal method: _plot_causal_graph_networkx(...), "
                                 "which does not depend on graphviz, but produces less nice plots.")
            else:
